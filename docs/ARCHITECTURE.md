...
`MovieLibraryStore` writes `library.json.tmp`, flushes it, deserializes it for validation, copies the previous valid library to `library.json.bak`, and then replaces `library.json`. If a legacy library is truncated, startup preserves the corrupt bytes and restores the valid backup. When no valid backup exists, a streaming JSON scan records the last fully completed item in the root `movies` array and rebuilds a valid document containing every complete movie; Studio reports the recovery and directs the user to Drive import for any incomplete final item.
