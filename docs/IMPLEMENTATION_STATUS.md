...
- Saves `library.json` through a validated temporary file and atomic replacement, retains the previous valid file as `library.json.bak`, and recovers from that backup or salvages every fully completed movie when a legacy direct-write file is truncated. The corrupt source is preserved with a timestamp instead of being discarded.
